...
my $Log = Log->new($db, \%conf, {
    debug    => $conf{TELEGRAM_DEBUG},
    LOG_FILE => $conf{TELEGRAM_LOG} || (($base_dir || '/usr/abills') . '/var/log/telegram.log'),
  });
...
sub start {
  ...
  $self->{config} = $config;
  $self->{debug} = $conf{TELEGRAM_API_DEBUG} || $debug;
  
  $Bot_API = Abills::Backend::Plugin::Telegram::BotAPI->new ({
    token => $self->{token}
  });
  ...
  $self->set_timer($config->{interval});
...
sub set_timer {
  ...
  $self->{timer} = AnyEvent->timer(
    ...
    interval => $interval,
    cb       => sub {
      $Log->log_print("LOG_ERROR", "[ Telegram ]", "Request");
      ...
      if ( $@ ) {
        $Log->log_print("LOG_ERROR", "[ Telegram ]", $@);
      }
    });
...
sub process_updates {
  ...
  foreach my $update ( @updates ) {
    ...
    print Dumper($update) if ( $self->{config}->{debug} > 4 );
    print Dumper($message) if ( $self->{config}->{debug} > 3 );
    ...
    if ( $message->{text} =~ /^\/start/ ) {
      if ( $message->{text} =~ /\/start ([ua])_([a-zA-Z0-9]+)/ ) {
        ...
        $Log->log_print("LOG_NOTICE", " Telegram ", "Auth  for $type $sid");
        
        if ( $self->authenticate($type, $sid, $chat_id) ) {
          $Log->log_print("LOG_DEBUG", " Telegram ", "Authorized $type $sid");
          $self->send_response("You've been registered", $chat_id);
          next;
        }
      }
      
      $Log->log_print("LOG_INFO", " Telegram ", "Auth failed  for $message->{from}->{username} ($chat_id)");
...
sub action_unknown_command {
  ...
  $Log->log_print("LOG_DEBUG", " Telegram ", "Don't know how should respond for: \n $message->{text} ");
